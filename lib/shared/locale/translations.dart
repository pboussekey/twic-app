...
const List<String> _supportedLanguages = ['en'];
...
class GlobalTranslations {
  ...
  ///
  String text(String key, [Map<String, dynamic> array]) {
    // Return the requested string
    array = array ?? _localizedValues;
    if(key.contains('.')){
      String subarray = key.substring(0, key.indexOf('.'));
      String subkey = key.substring(key.indexOf('.') + 1);
      return array[subarray] == null  ? '** $key not found' : text(subkey, array[subarray]);
    }
    return (array == null || array[key] == null) ? '** $key not found' : array[key];
  }
